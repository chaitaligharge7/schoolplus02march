...
export class PlanListComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    { key: 'plan_title', label: 'Title', sortable: true, format: (v: any) => v || '-' },
    { key: 'class_name', label: 'Class', sortable: true, format: (v: any) => v || '-' },
    { key: 'subject_name', label: 'Subject', sortable: true, format: (v: any) => v || '-' },
    { key: 'teacher_name', label: 'Teacher', sortable: true, format: (v: any) => v || '-' },
    { key: 'plan_date', label: 'Date', sortable: true, type: 'date', format: (v: any) => v || '-' },
    { key: 'status', label: 'Status', sortable: true, format: (v: any) => `<span class="badge ${v === 'active' ? 'badge-success' : 'badge-secondary'}">${v || '-'}</span>` }
  ];
  searchTerm = '';

  constructor(private planService: TeachingPlanService, 
  ...
  load(): void {
    ...
    this.planService.getPlans({ page: this.pagination.page, limit: this.pagination.limit, search: this.searchTerm }).subscribe({
      ...
      error: () => { this.loading = false; this.toast.show('Error loading plans', 'error'); }
    });
  }
